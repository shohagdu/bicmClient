        ...
        <main class="container">
            <div class="row">
                ...
                <div class="col-md-8">
                    <article>
					    <br/>
                        <h3 class="article-title" style="Text-align: justify;  font-family: Times New Roman; font-size: 20px; color:#000; ">All Rejected Paper</h3>
						...
						<p>
						    ...
						    <?php
							           if($_SESSION['type']==4){
										$queryData=$db->query("SELECT * FROM sr_menuscript_info A,sr_document_info B WHERE  A.paperUniqID=B.paperUniqID_Doc AND A.editor_status IN(11) ORDER BY A.id DESC");
									   }else if($_SESSION['type']==2) {
										   $queryData=$db->query("SELECT * FROM sr_menuscript_info A, sr_document_info B,sr_editor_reviwer_assign_info C WHERE  A.paperUniqID=B.paperUniqID_Doc AND A.paperUniqID=C.paperId AND C.assignID='$pid' AND A.editor2_status IN (2) ORDER BY A.id DESC");
                                       
									   }
									   else if($_SESSION['type']==3) {
										   	$queryData=$db->query("SELECT * FROM sr_menuscript_info A, sr_document_info B,sr_editor_reviwer_assign_info C WHERE  A.paperUniqID=B.paperUniqID_Doc AND A.paperUniqID=C.paperId AND C.assignID='$pid' AND A.reviwer_status IN (2) ORDER BY A.id DESC");
