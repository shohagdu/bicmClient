   ...
   $row = $query->fetch_assoc();
    ...
    $QueryEditRemove=$db->query("DELETE FROM sr_menuscript_info WHERE paperUniqID='$DELETEID'");
        if($QueryEditRemove==1)
        {
            ...
            header("location:all_submission_reviewer.php?mgsalt=10");
        ...
        <main class="container">
            <div class="row">
                ...
                <div class="col-md-8">
                    ...
                    <article>
					    <br/>
                        <h3 class="article-title" style="text-align: justify;  font-size: 18px; color: black; ">All under review papers</h3>
						...
						<p>
						    ...
						    <?php
							           if($_SESSION['type']==4){
										$queryData=$db->query("SELECT * FROM sr_menuscript_info A,sr_document_info B WHERE  A.paperUniqID=B.paperUniqID_Doc AND A.status in (2,3) ORDER BY A.id DESC");
									   }
							...
							<div style="border:1px solid gray; padding:10px; font-size: 15px; color:#000;">
							        <p align="right">
							            <?php  if($_SESSION['type']==4){  ?>
							              <a href="all_submission_reviewer.php?DELETEID=<?php echo $paperUniqIDForDataGet;  ?>"><button style="width:200px; font-size:12px; height:30px; color: white; background: black;">Remove to this paper </button> </a>
							           <?php } ?>
