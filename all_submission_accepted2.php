                ...
                <div class="col-md-8">
                    <article>
						...
						<p>
						    ...
						    <?php
							           if($_SESSION['type']==4){
										$queryData=$db->query("SELECT * FROM sr_menuscript_info A,sr_document_info B WHERE  A.paperUniqID=B.paperUniqID_Doc AND A.editor_status IN(10) ORDER BY A.id DESC");
									   }else if($_SESSION['type']==2) {
										   $queryData=$db->query("SELECT * FROM sr_menuscript_info A, sr_document_info B,sr_editor_reviwer_assign_info C WHERE  A.paperUniqID=B.paperUniqID_Doc AND A.paperUniqID=C.paperId AND C.assignID='$pid' AND A.editor2_status IN (1) ORDER BY A.id DESC");
                                       
									   }
									   else if($_SESSION['type']==3) {
										   	$queryData=$db->query("SELECT * FROM sr_menuscript_info A, sr_document_info B,sr_editor_reviwer_assign_info C WHERE  A.paperUniqID=B.paperUniqID_Doc AND A.paperUniqID=C.paperId AND C.assignID='$pid' AND A.reviwer_status IN (1) ORDER BY A.id DESC");
